...
export function useSessionSalesQuery(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<{
      ...
      // Parsear payment_totals si es una cadena JSON
      let paymentTotals: PaymentTotals | null = null;
      if (session.payment_totals) {
        try {
          if (typeof session.payment_totals === 'string') {
            paymentTotals = JSON.parse(session.payment_totals) as PaymentTotals;
          } else {
            paymentTotals = session.payment_totals as PaymentTotals;
          }
        } catch (e) {
          console.error('Error al parsear payment_totals:', e);
          paymentTotals = null;
        }
      }
