...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Redirigir según el rol - SOLO PERMITIR OWNERS
      if (('isOwner' in result && result.isOwner) || result.role === 'owner') {
        // Owner → redirigir a /dashboard/subscription
        toast.success(`¡Bienvenido! Accediendo al dashboard`);
        setIsLoading(false);
        ...
        // Redirigir a la página de suscripción después del login
        if (typeof window !== 'undefined') {
          window.location.href = "/dashboard/subscription";
        }
      } else {
        console.log('[LoginForm] Usuario no es owner - acceso denegado');
        // Usuario sin permisos de owner - DENEGAR ACCESO
